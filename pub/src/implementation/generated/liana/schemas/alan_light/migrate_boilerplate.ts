...
export const Node: t_signatures.Node = ($) => ({
    'properties': _p.deprecated_cc($['properties'], ($) => $.__d_map(($) => ({
        'type': _p.deprecated_cc($['type'], ($) => _p.deprecated_cc($, ($): t_out.Node.properties.D.type_ => {
            switch ($[0]) {
                case 'collection': return _p.ss($, ($) => ['collection', ({
                ...
                case 'state group': return _p.ss($, ($) => ['state group', ({
                    'states': _p.deprecated_cc($['states'], ($) => $.__d_map(($) => ({
                        'constraints': _p.deprecated_cc($['constraints'], ($) => $.__d_map(($) => ({
                            'path': _p.deprecated_cc($['path'], ($) => Path(
                                $
                            )),
                        }))),
...
export const Root: t_signatures.Root = ($) => ({
    'numerical types': _p.deprecated_cc($['numerical types'], ($) => $.__d_map(($) => null)),
